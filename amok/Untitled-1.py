...
def save_note():
    if list_notes.selectedItems():
        ...
        print("please_choose:showerror;glory;to;jesus;christ--no_allah,jesus,onlyJesus")
...
def del_note():
    if list_notes.selectedItems():
        key = list_notes.selectedItems()[0].text()
        del notes[key]
        list_notes.clear(); list_tags.clear(); field_text.clear(); list_notes.addItems(notes)
        with open("notes_data.json", "w") as file:
            json.dump(notes, file, sort_keys=True, ensure_ascii=False)
        print(notes)
    else:
        print("not choosed error 404 except:SOS;except:SOS;line:YoMo;help;help;help")
